...
- **What operators get** - operators get a plugin settings page, migration preview/status data, a per-issue migration mapping preview, repair tools, and an issue-level `Memory` tab.
...
Set `honchoApiKey` first. If you are not using Honcho Cloud, also set `honchoApiBaseUrl`. Everything else can be left at the defaults until you have a reason to change it.
...
| `workspacePrefix` | `paperclip` | Fallback namespace used when a company name is unavailable. |
